...
class OrderController extends Controller {
	...
	public function ordersAjax(Request $request) {
		...
		foreach ($orders as $order) {
			...
			$data['user_id'] = !empty($order->user) ? $order->user['first_name'] . " " . $order->user['last_name'] : '';
			$data['payment_method'] = Config::get('constants.PAYMENT_METHOD.' . $order->payment_method);
			$data['address'] = !empty($order->user_address) ? $order->user_address['first_name'] . " " . $order->user_address['last_name'] . ", " . $order->user_address['apartment'] . ", " . $order->user_address['address'] . ", " . $order->user_address['acity']['name'] . ", " . $order->user_address['astate']['name'] . ", " . $order->user_address['acountry']['name'] . "- " . $order->user_address['zipcode'] : '';
			...
			if (Auth::guard('admin')->user()->user_role[0]->role == 'user' || Helper::checkAccess(route('viewOrder'))) {
				...
				if (Auth::guard('admin')->user()->user_role[0]->role != 'user' && in_array($order->status, ['PN', 'CM'])) {
