...
<section class="wrapperlisting1">
    <div class="container">
        <?php
if ($banner && File::exists(public_path('uploads/categories/' . $category->banner))) {
	$banner = $banner;
	$background = 'cover';
} else {
...
?>
         ...
         <div class="listingBanner text-center py-5" style="background-image:url(<?=$banner?>);background-size:<?=$background?>">
            {{-- <div class="font60 colorWhite font-weight-bold bestOffer">Best Offer</div>
            <div class="font60 colorWhite font-weight-bold onVeg mt-4">On Vegetables</div> --}}
        </div>
    </div>
</section>
...
<section class="wrapper2 quickGrab bgf7 py-3 py-md-5">
    <div class="container">
        <div class="sectionTitleBlock d-flex mb-5">
            <h5 class="sectionTitle font35 font-weight-bold color11">{{ isset($category) ? ucfirst($category->name) : 'Product' }}</h5>
            <div class="filter_dropdown position-relative" style="margin-left: auto;">
            <a class="filter_btn shadow" href="javascript:;" id="sortby">Sort by </a>
            <a href="javascript:;" id="clearsort" style="float: right;margin-top: 5px;display: none;">Clear</a>
        ...
        <div class="alert1 alert-success d-none mb-5">
            <ul>
...
?>
            <div class="col-md-4 mb-4" style="max-height: 300px;">
                    <div class="items" style="height: 100%;">
                        <div class="productImg">
                            <a href="{{ url('product/grocery/'.$product->id) }}" target="_blank"><img src="{{$medium_image}}" alt="product" height=100 style="width:auto !important;" data-testid="productimg"></a>
                        </div>
                        <h5 class="productName font18 color26 mb-1 fontsemibold text-center mt-3" data-testid="productname">{{ $product->name }}</h5>
                        <div class="d-flex justify-content-around">
                            ...
                            <p class="price"><span class="newPrice font18 color20" data-testid="new-price">{{ $currency }}{{ $product->price }}</p>
                            @else
                            <p class="price"><span class="newPrice font18 color20" data-testid="new-price">{{ $currency }}{{ $product->special_price }}</span>
                            <span class="oldPrice font18 colorb7" data-testid="old-price"><strike>{{ $currency }}
                        ...
                        <div class="favtag">
                            <p class="font20 fontsemibold mb-0">
                                <a class="{{ $product->is_favorite==1 ? 'favdark' : '' }}" href="javascript:void(0)" data-product="{{ $product->id }}" onclick="addToFavourite(this,{{ $product->product_variation_id }},'fav')" data-testid="favorites">
        ...
        {{ count($products)>0 ? $products->links("pagination::bootstrap-4") : '' }}
    </div>
</section>
@endsection
