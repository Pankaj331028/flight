...
?>
                            ...
                            <div class="orderDetailDesc">
                                ...
                                <p class="font16 color20 fontsemibold mb-1 d-flex align-items-center">
                                    @if($product->special_price == $product->price || $product->special_price=='0')
                                        <span data-testid="new-price">{{ $item['currency'] }}{{ $product->price }}</span>
                                    @else
                                    <span data-testid="new-price">{{ $item['currency'] }}{{ $product->special_price }}</span>
                                    <strike  data-testid="old-price" class="colorb7 ml-2">{{ $item['currency'] }}{{ $product->price }}</strike>
                                    @endif
                                </p>
            ...
            <div class="col-md-6">
                <div class="orderDetailCalender d-flex align-items-center justify-content-between">
                  @if($product->status=='pending')
                    <div class="btn_or_1">
                        <button type="submit" class="cancelOrder font16 btn_trans colorTheme border1 radius50 px-4 py-2 d-flex align-items-center" data-testid="cancel-order">Cancel</button>
                  </div>
                  <div class="btn_or_1">
