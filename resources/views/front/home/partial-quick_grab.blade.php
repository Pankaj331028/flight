<section class="wrapper2 render-QG quickGrab bgf7 py-3 py-md-5">
    <div class="container">
        <div class="sectionTitleBlock d-flex justify-content-between align-items-center mb-5">
            <h5 class="sectionTitle font35 font-weight-bold color11">Quick Grabs</h5>
...
$str = public_path(substr($quick_grab->small_image, strpos($quick_grab->small_image, 'uploads')));
            ...
            <div class="items">
                ...
                <div class="d-flex justify-content-around">
                    @if($quick_grab->price == $quick_grab->special_price || $quick_grab->special_price=='0')
                        <p class="price"><span class="newPrice font18 color20" data-testid="new-price">{{ $currency }}{{ $quick_grab->price }}</span>
                    @else
                        <p class="price"><span class="newPrice font18 color20" data-testid="new-price">{{ $currency }}{{ $quick_grab->special_price }}</span>
                        <span class="oldPrice font18 colorb7" data-testid="old-price"><strike>{{ $currency }}{{ $quick_grab->price }}</strike></span>
                    </p>
                    @endif
                ...
                <div class="favtag">
                    <p class="font20 fontsemibold mb-0">
                        <a class="{{ $quick_grab->is_favorite==1 ? 'favdark' : '' }}" href="javascript:void(0)" data-product="{{ $quick_grab->id }}" onclick="addToFavourite(this,{{ $quick_grab->product_variation_id }},'fav')" data-testid="favorites">
