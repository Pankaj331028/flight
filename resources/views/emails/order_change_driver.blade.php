	...
	<table width="800" cellspacing="0" style="border:1px solid #40b851; border-radius:5px; overflow:hidden">
		...
		<tr>
			<td style="text-align:left; padding:10px 0px 20px 20px; " bgcolor="#f9f9f9">
				<div style="margin:20px auto 10px; font-size:18px;"><h4>New Order</h4></div>
				<div style="margin:20px auto 10px; font-size:28px;">Hello {{$first_name." ".$last_name}}, </div>
				<div style="margin:20px auto 10px; font-size:15px;">Order #: {{$order->order_no}} has been assigned to other driver for date {{date('d M, Y', strtotime($date)) }}. You need not go to the warehouse to pickup the order.</div>
			</td>
		</tr>
		<tr>
