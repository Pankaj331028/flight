	...
	<table width="800" cellspacing="0" style="border:1px solid #40b851; border-radius:5px; overflow:hidden">
		...
		<tr>
			<td style="text-align:left; padding:10px 0px 20px 20px; " bgcolor="#f9f9f9">
				...
				<div style="margin:20px auto 10px; font-size:15px;">omrbranch Admin has created an account using your email id: {{$email}} and assigned you {{$role}} role. Please install {{ucfirst($role)}} app and login with the following credentials:<br><br><strong>Email-ID: </strong>{{$email}}<br><strong>Password: </strong>{{$password}}</div>
